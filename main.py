                            ...
                            try:
                                ...
                                if self.lock_id is not None:
                                    ...
                                    try:
                                        ...
                                        cv2.imshow(f'ID:{self.lock_id}',result_cropped)
                                        cv2.moveWindow(f'ID:{self.lock_id}', 0, 0)
                                        #press esc to quit
    ...
    def show_traffic_graph(self):#加载流量折线图（屎山XD）
        if self.yolo_predict.run_started:
            if self.yolo_predict.show_graph is True and not self.yolo_predict.X_quit:
                ...
                MessageBox(
                    self.close_button, title='提示', text='正在停止继续加载行人流量折线图...', time=500, auto=True).exec()
                return
            MessageBox(
                    self.close_button, title='提示', text='正在加载行人折线图...', time=500, auto=True).exec()
